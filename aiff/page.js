...
const Mp3Converter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.aiff']);
            
            const data = await ffmpeg.readFile('output.aiff');
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/aiff' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.aiff');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to AIFF</button>
                    )}
                </div>
            )}
        </div>
