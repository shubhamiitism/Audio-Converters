...
const opusConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.opus']);
            
            const data = await ffmpeg.readFile('output.opus');
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/opus' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.opus');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to opus</button>
                    )}
                </div>
            )}
        </div>
