...
const flacConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/flac' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.flac');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to flac</button>
                    )}
                </div>
            )}
        </div>
