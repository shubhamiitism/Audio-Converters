...
const m4aConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/m4a' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.m4a');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to m4a</button>
                    )}
                </div>
            )}
        </div>
