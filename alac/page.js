...
const cafConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.caf']);
            
            const data = await ffmpeg.readFile('output.caf');
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/caf' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.caf');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to caf</button>
                    )}
                </div>
            )}
        </div>
