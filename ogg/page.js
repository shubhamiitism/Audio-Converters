...
const oggConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/ogg' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.ogg');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to ogg</button>
                    )}
                </div>
            )}
        </div>
