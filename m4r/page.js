...
const m4rConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.m4r']);
            
            const data = await ffmpeg.readFile('output.m4r');
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/m4r' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.m4r');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to m4r</button>
                    )}
                </div>
            )}
        </div>
