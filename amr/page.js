...
const amrConverter = () => {
    ...
    const transcode = async () => {
        ...
        try {
            ...
            await ffmpeg.exec(['-i', 'input', 'output.amr']);
            
            const data = await ffmpeg.readFile('output.amr');
            const audioURL = URL.createObjectURL(new Blob([data.buffer], { type: 'audio/amr' }));
            setDownloadLink(audioURL);

            // Automatically trigger download
            triggerDownload(audioURL, 'output.amr');
        } catch (error) {
            ...
            {loaded && (
                <div className="actions">
                    {processing ? (
                        ...
                        <button className="convert-btn" onClick={transcode}>Convert to amr</button>
                    )}
                </div>
            )}
        </div>
